...
int elf64_load(tori::vfs::Vnode *file, uint64_t pml4_phys, ElfLoadResult *out) {
  ...
  phdrs = static_cast<Elf64_Phdr *>(
      tori::memory::address::physical_to_virtual(phdr_buf_phys));
  ...
  for (uint16_t i = 0; i < ehdr.e_phnum; ++i) {
    ...
    for (uint64_t page = page_start; page < page_end; page += 0x1000) {
      ...
      auto flags =
          tori::memory::vmm::Flags::Present | tori::memory::vmm::Flags::User;
      ...
      if (remaining_file > 0) {
        ...
        if (copy_size > 0) {
          got = 0;
          err = file->ops->read(file, file_offset, hhdm + page_off, copy_size,
                                &got);
          if (err < 0) {
  ...
  uint64_t last_stack_phys = 0;
  {
    ...
    for (uint64_t page = user_stack_base; page < user_stack_top;
         page += 0x1000) {
      uint64_t phys = tori::memory::pmm::alloc_page();
      if (phys == tori::memory::pmm::invalid_physical_address) {
