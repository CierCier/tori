...
void log_boot_info(const tori::boot::BootInfo &boot_info) {
  ...
  TORI_LOG_TEXT_VALUE(tori::log::Level::Info, "boot", "bootloader version",
                      boot_info.bootloader_version);
  TORI_LOG_TEXT_VALUE(tori::log::Level::Info, "boot", "command line",
                      boot_info.command_line);
  TORI_LOG_VALUE(tori::log::Level::Info, "boot", "kernel physical base",
                 boot_info.kernel_address.physical_base);
  TORI_LOG_VALUE(tori::log::Level::Info, "boot", "kernel virtual base",
                 boot_info.kernel_address.virtual_base);
  TORI_LOG_VALUE(tori::log::Level::Info, "boot", "hhdm offset",
                 boot_info.hhdm_offset);
  TORI_LOG_VALUE(tori::log::Level::Info, "boot", "rsdp",
                 reinterpret_cast<uint64_t>(boot_info.rsdp));
  TORI_LOG_VALUE(tori::log::Level::Info, "boot", "module count",
                 boot_info.module_count);

  if (boot_info.has_framebuffer) {
    TORI_LOG_VALUE(tori::log::Level::Info, "fb", "width",
                   boot_info.framebuffer.width);
    TORI_LOG_VALUE(tori::log::Level::Info, "fb", "height",
                   boot_info.framebuffer.height);
    TORI_LOG_VALUE(tori::log::Level::Info, "fb", "pitch",
                   boot_info.framebuffer.pitch);
    TORI_LOG_VALUE(tori::log::Level::Info, "fb", "bpp",
                   boot_info.framebuffer.bits_per_pixel);
  } else {
...
void log_memory_summary(const tori::boot::MemoryMap &memory_map) {
  const tori::memory::MemorySummary summary =
      tori::memory::summarize(memory_map);

  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "region count",
                 memory_map.region_count);
  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "total bytes",
                 summary.total_bytes);
  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "usable bytes",
                 summary.usable_bytes);
  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "bootloader reclaimable bytes",
                 summary.reclaimable_bytes);
  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "reserved bytes",
                 summary.reserved_bytes);
  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "framebuffer bytes",
                 summary.framebuffer_bytes);
  TORI_LOG_VALUE(tori::log::Level::Info, "mem", "invalid regions",
                 summary.invalid_regions);
}

void log_madt_info(const tori::acpi::madt::Info &info) {
  TORI_LOG_VALUE(tori::log::Level::Info, "madt", "local apic base",
                 info.local_apic_address);
  TORI_LOG_VALUE(tori::log::Level::Info, "madt", "usable CPUs",
                 info.local_apic_count);
  TORI_LOG_VALUE(tori::log::Level::Info, "madt", "IOAPIC count",
                 info.io_apic_count);
  TORI_LOG_VALUE(tori::log::Level::Info, "madt", "interrupt overrides",
...
void init_acpi(const tori::boot::BootInfo &boot_info) {
  ...
  TORI_LOG_VALUE(tori::log::Level::Info, "acpi", "xsdt entry count",
                 info.xsdt_entry_count);
  TORI_LOG_VALUE(tori::log::Level::Info, "acpi", "madt present",
                 info.madt_found ? 1ULL : 0ULL);
  TORI_LOG_VALUE(tori::log::Level::Info, "acpi", "fadt present",
                 info.fadt_found ? 1ULL : 0ULL);
  TORI_LOG_VALUE(tori::log::Level::Info, "acpi", "hpet present",
                 info.hpet_found ? 1ULL : 0ULL);

  if (info.madt_found) {
    const tori::acpi::SDTHeader *madt_header =
        tori::acpi::find_table(rsdp, "APIC");
    if (madt_header != nullptr) {
      const tori::acpi::madt::Info madt_info =
          tori::acpi::madt::parse(madt_header);
...
void log_vmem_layout() {
  TORI_LOG_VALUE(tori::log::Level::Info, "vmem", "kernel image start",
                 tori::memory::vmem::kernel_image_start());
  TORI_LOG_VALUE(tori::log::Level::Info, "vmem", "kernel image size",
                 tori::memory::vmem::kernel_image_size());
  TORI_LOG_VALUE(tori::log::Level::Info, "vmem", "kernel image end",
                 tori::memory::vmem::kernel_image_end());
}
...
tori::boot::MemoryMap copy_memory_map(const tori::boot::MemoryMap &source) {
  ...
  auto *regions = static_cast<tori::boot::MemoryRegion *>(
      tori::memory::kalloc(bytes, alignof(tori::boot::MemoryRegion)));
  if (regions == nullptr) {
...
int populate_ramfs_from_fat(tori::vfs::Vnode *fat_dir,
                            tori::vfs::Vnode *ramfs_dir) {
  uint64_t offset = 0;
    ...
    if (entry.type == tori::vfs::VnodeType::Directory) {
      ...
      if (err == 0) {
        ...
        do {
          ...
          got = 0;
          err =
              fat_child->ops->read(fat_child, file_off, buf, sizeof(buf), &got);
          if (err == 0 && got > 0) {
            size_t written = 0;
...
[[noreturn]] void kernel_main(const boot::BootInfo &boot_info) {
  ...
  if (owned_boot_info.memory_map.region_count == 0 ||
      (owned_boot_info.memory_map.regions == nullptr &&
       owned_boot_info.memory_map.source_region_at == nullptr)) {
  ...
  const tori::acpi::SDTHeader *madt_header = tori::acpi::find_table(
      static_cast<const tori::acpi::RSDP *>(owned_boot_info.rsdp), "APIC");
  ...
  // For now, let's assume the BSP is the first CPU in the list that matches
  // bsp_lapic_id.
  size_t bsp_index = 0;
  if (owned_boot_info.has_smp) {
    for (size_t i = 0; i < owned_boot_info.smp.cpu_count; ++i) {
      if (owned_boot_info.smp.cpus[i].lapic_id ==
          owned_boot_info.smp.bsp_lapic_id) {
        bsp_index = i;
        break;
      }
    }
  ...
  tori::vfs::Vnode* lower_root = nullptr;
  {
    ...
    for (uint64_t i = 0; i < owned_boot_info.module_count; ++i) {
      ...
      auto *membdev = tori::block::memblock_create(
          reinterpret_cast<const void *>(mod.address),
          static_cast<size_t>(mod.size));
      if (!membdev) {
  ...
  if (owned_boot_info.has_smp) {
    ...
    for (size_t i = 0; i < owned_boot_info.smp.cpu_count; ++i) {
      ...
      TORI_LOG_VALUE(tori::log::Level::Info, "kernel", "ap lapic id",
                     owned_boot_info.smp.cpus[i].lapic_id);
  ...
  TORI_LOG_INFO(
      "kernel",
      "boot, memory, ACPI, and CPU runtime initialized; interrupts enabled");

  tori::proc::init_syscall();

  TORI_LOG_INFO("kernel", "starting scheduler");

  tori::sched::Task *main_task =
      tori::sched::create_task(kernel_main_task, nullptr, "kernel-main");
  if (main_task == nullptr) {
    TORI_PANIC("kernel", "failed to create kernel main task");
  }

  TORI_LOG_VALUE(tori::log::Level::Info, "kernel", "main task id",
                 main_task->id);
...
void kernel_main_task(void *) {
  ...
  if (init_pid < 0) {
    TORI_LOG_ERROR("kernel", "failed to spawn /sys/init");
    TORI_LOG_VALUE(tori::log::Level::Error, "kernel", "error code",
                   static_cast<uint64_t>(-init_pid));
    TORI_PANIC("kernel", "failed to spawn /sys/init");
  }
